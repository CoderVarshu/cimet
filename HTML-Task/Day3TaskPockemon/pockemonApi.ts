...
const showPockemon: HTMLDivElement = document.querySelector(".showPockemon");
const loadBtn: HTMLButtonElement  = document.querySelector(".loadMoreBtn")!;
...
async function FetchPockemon() : Promise<void> {
  try {
    ...
    let fetchPromise : Promise<PokemonResponse>[] = response.results.map(async (item) => {
